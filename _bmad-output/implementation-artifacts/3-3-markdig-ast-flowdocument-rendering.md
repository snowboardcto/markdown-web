...
- [x] **Task 1 — Add `Xunit.StaFact` + DisableTestParallelization to `Rendering.Tests` (AC: 12)**
  - [x] In `clients/windows/Rendering.Tests/TheMarkdownWeb.Rendering.Tests.csproj`, add `<PackageReference Include="Xunit.StaFact" Version="1.1.11" />` (match `App.Tests`). Add NO other package. Keep the existing `Microsoft.NET.Test.Sdk` 17.12.0 / `xunit` 2.9.2 / `xunit.runner.visualstudio` 2.8.2 and the `ProjectReference` to `Rendering` unchanged. [Source: Environment Constraint STA; App.Tests csproj as the version template; AC12]
  ...
  - [x] Confirm `NoEmbeddedBrowserTests` (in App.Tests) still passes: `Xunit.StaFact` contains no forbidden embedded-browser substring, so the csproj-tier glob over `Rendering.Tests.csproj` stays green. Add NO forbidden dependency. [Source: AC11; NoEmbeddedBrowserTests globs clients/windows/**/*.csproj]

- [x] **Task 2 — `FlowDocumentRenderer` skeleton: the API surface + shared Markdig GFM pipeline + FlowDocument root (AC: 1, 11)**
...
- [x] **Task 11 — Map images → `Image` element + recorded source/alt, NO network fetch (AC: 10, 11)**
  - [x] Map Markdig `LinkInline` with `IsImage == true` → a `System.Windows.Controls.Image` hosted in an `InlineUIContainer` (inline) or `BlockUIContainer` (standalone image paragraph). Record the source URI on the element (`Image.Tag = url`, and/or `Image.Source = new BitmapImage` whose `UriSource` is the absolute URI **configured for lazy/delayed load** so no synchronous download happens) and set `AutomationProperties.SetName(image, altText)`. **Open NO socket; hold NO `HttpClient`; do NOT block on a download.** A relative/missing/unresolvable source does NOT throw — it yields the `Image` element with the recorded source + alt and a graceful empty state (actual fetch/resolve is Story 3.5). [Source: AC10, AC11; architecture image mapping with vault-resolution deferred to FR-3/3.5; Resolved image-loading decision]
  - [x] **`[StaFact]` AC10:** render `"![alt](https://host/pic.png)"`; assert the container's child is an `Image`, its recorded source URI == `https://host/pic.png`, its automation name == `alt`, **and no network access occurs** (the test opens no socket; a created `BitmapImage` is not downloaded/decoded by the test). Render an image with a relative/unresolvable source → no throw, element still produced. [Source: AC10]

- [x] **Task 11b — Edge-case / totality hardening tests (decisions D1–D9) (AC: 1, 3, 5, 9, 11)**
  - [x] **`[StaFact]` totality (D9):** a `[Theory]`/`[StaFact]` over malformed inputs (unterminated ```` ``` ````, ragged `| a |\n|---|`, stray `~`/`>`/`|`, `<div>x</div>`, control chars, whitespace-only) — each `Render(...)` returns non-null and does NOT throw. [Source: D9; AC1]
  - [x] **`[StaFact]` raw HTML (D2):** render `"<div>x</div>"` and `"a <b>x</b> b"` → the literal HTML appears as plain-text `Run`(s); assert NO WPF webview/HTML type is produced; no throw. (Reinforces AC11 no-embedded-browser.) [Source: D2; AC11]
  - [x] **`[StaFact]` thematic break (D1):** render `"a\n\n---\n\nb"` → a separator block tagged `Tag == "hr"` between the two paragraphs; no throw. [Source: D1]
  - [x] **`[StaFact]` nesting (D3):** list-in-blockquote (`"> - a\n> - b"`) → `Section` containing a `List`; code-in-list (`"- ```\n  x\n  ```"` equivalent) → a mono code block inside a `ListItem`; table cell with `**bold**` → the cell's paragraph carries a bold inline; 3-deep nested list reachable. [Source: D3]
  - [x] **`[StaFact]` nested emphasis (D3):** `"***x***"` → inner `Run` is BOTH bold AND italic. [Source: D3; AC3]
  - [x] **`[StaFact]` H7 is not a heading (D6):** `"####### x"` → an untagged `Paragraph` (NOT `h7`/any heading), proving the H1–H6-only invariant. [Source: D6; AC2]
  - [x] **`[StaFact]` fenced corner cases (D7):** empty fence (` ``` \n ``` `) and no-language fence render mono with null/empty `Tag`, no throw; backticks-in-content preserved verbatim. [Source: D7; AC5]
  - [x] **`[StaFact]` ragged/header-only table (D8):** a header row with NO body rows, and a body row with a different cell count than the header, each render a valid `Table` without throwing. [Source: D8; AC9]
  - [x] **`[StaFact]` image edge cases:** no-alt (`![](https://h/p.png)`) → empty automation name, source recorded, no throw; relative src (`![a](pic.png)`) → element produced, no throw, NO socket. [Source: AC10]
  - [x] (Optional) hard line break (D4): `"a  \nb"` → a `LineBreak` inline; autolink (D5): a raw URL → an inert `Hyperlink`. [Source: D4, D5]

- [x] **Task 12 — Boundary / purity / no-Chromium hygiene (AC: 11, 12)**
  - [x] Confirm `Rendering` added NO `PackageReference` beyond the existing `Markdig 1.3.1`, NO `System.Net.Http`, NO AI, NO ref to `App`/`Agent`; the csproj TFM/`UseWPF` are UNCHANGED. The inherited `DependencyBoundaryTests.Rendering_DoesNotReference_AppOrAgent` and `NoEmbeddedBrowserTests` (csproj tier over `Rendering`/`Rendering.Tests`) stay green. [Source: AC11; DependencyBoundaryTests; NoEmbeddedBrowserTests]
  - [x] Confirm this story touched ONLY `Rendering/*` (new render code) and `Rendering.Tests/*` (new tests + `Xunit.StaFact` + `AssemblyInfo.cs`). `App`/`Agent`/`App.Tests`/`TheMarkdownWeb.sln`/`build-windows.yml` are UNTOUCHED. `Rendering.Tests` is already in the `.sln`; the CI `paths:` filter (`clients/windows/**`) already covers the new files — **no `.sln` or workflow edit.** [Source: AC12; .sln already lists Rendering.Tests; build-windows.yml paths]
  - [x] Keep `nullable`/`ImplicitUsings` consistent; introduce no new build warnings. The new render code must compile clean alongside the existing `MarkdownRenderer`. [Source: AC12; existing csproj conventions]

- [x] **Task 13 — Final verification against ACs (Definition of Done — checked via CI, not locally) (AC: 1–12)**
  - [x] **AC1:** `FlowDocumentRenderer.Render` returns a `FlowDocument`; empty→empty, null→throws, GFM pipeline (`UseAdvancedExtensions`) built once. Proven by the AC1 `[StaFact]`.
  - [x] **AC2:** H1–H6 → bold `Paragraph`s, monotonic size, level marker. Proven by the AC2 `[StaFact]`.
  - [x] **AC3:** bold/italic/strikethrough → `Bold`/`Italic`/`TextDecorations.Strikethrough`. Proven by the AC3 `[StaFact]`.
  - [x] **AC4:** paragraph + inline code (mono `FontFamily`). Proven by the AC4 `[StaFact]`.
  - [x] **AC5:** fenced code → mono block, language preserved in `Tag`, SINGLE foreground (no colors — 3.4). Proven by the AC5 `[StaFact]`.
  - [x] **AC6:** ordered/unordered/nested lists → `List`+`ListItem`. Proven by the AC6 `[StaFact]`.
  - [x] **AC7:** task lists → read-only checkbox per item reflecting checked state. Proven by the AC7 `[StaFact]`.
  - [x] **AC8:** blockquote → `Section` with a left rule. Proven by the AC8 `[StaFact]`.
  - [x] **AC9:** GFM table → `Table` with correct columns/rows + header. Proven by the AC9 `[StaFact]` (+ optional pure `[Fact]`).
  - [x] **AC10:** image → `Image` element + recorded source/alt, NO network fetch in `Rendering`. Proven by the AC10 `[StaFact]` (+ Task 11b no-alt/relative-src cases).
  - [x] **Edge-case totality (D1–D9):** Task 11b `[StaFact]`s — malformed inputs never throw (D9); raw HTML → literal text, no webview (D2); `Tag="hr"` separator (D1); nesting + nested emphasis (D3); `####### x` not a heading (D6); empty/no-lang fence (D7); ragged/header-only table (D8). Proven by the Task 11b hardening tests.
  - [x] **AC11:** `Rendering` pure — no net/AI/reverse-ref; TFM/`UseWPF` unchanged; guards green. Proven by inherited `DependencyBoundaryTests` + `NoEmbeddedBrowserTests` staying green.
  - [x] **AC12:** `dotnet build -c Release` clean + `dotnet test -c Release` all green on `windows-latest` (existing `Rendering.Tests` + `App.Tests` + new render tests); STA + `DisableTestParallelization`; no shown window/socket/pixel/timing. Proven by the green `build-windows.yml` run.
...
- Implemented the new `FlowDocumentRenderer` + `FlowDocumentRenderOptions` types in the existing pure `Rendering` library (Markdig already referenced; no new packages; TFM/`UseWPF` untouched). `MarkdownRenderer` left intact.
- Shared Markdig pipeline built once as a `private static readonly` field via `new MarkdownPipelineBuilder().UseAdvancedExtensions().Build()` (pipe tables + task lists + strikethrough + autolinks). `Render` is total: `ArgumentNullException.ThrowIfNull` only for null; any other string → valid `FlowDocument`, never throws (mutually-recursive block/inline mappers, defensive default cases).
- Document baseline: `FontFamily = BodyFontFamily` ("Segoe UI"), `FontSize = 14`. Heading sizes 30/24/20/18/16/15 are strictly monotonic and all > 14. Headings carry `Tag="h1".."h6"`, `FontWeight=Bold`. `####### x` parses as a paragraph → `Tag=null` (Markdig caps at H6).
- Emphasis: strong (DelimiterCount>=2) → `Bold`; emphasis → `Italic`; strikethrough (DelimiterChar=='~') → `Span` with `TextDecorations.Strikethrough`. Nesting composes via WPF property-value inheritance (inner `Run` reports both Bold + Italic for `***x***`/`**_x_**`).
- Inline code / fenced code use the mono `FontFamily` (default "Consolas"). Fenced block is a single `Paragraph` with `Tag` = info-string first word (or null when absent), one `Run` per source line separated by `LineBreak`, verbatim text. NO `Foreground` is set on any code run, so `DistinctForegrounds` == 0 (<=1) — proves no Story-3.4 colors.
- Lists: `List` with `Disc`/`Decimal` MarkerStyle (+ `StartIndex` for ordered), recursive item/nested-list mapping. Code-block-in-list and 3-deep nesting reachable.
- **Task-list representation chosen: a read-only `System.Windows.Controls.CheckBox` (`IsChecked` reflects state, `IsEnabled=false`) hosted in an `InlineUIContainer` prepended to the item's first paragraph.** (NOT the glyph option.)
- Blockquote → `Section` with `BorderThickness.Left=4` + a non-null `BorderBrush` + left `Padding`; nested quotes nest Section-in-Section.
- GFM tables → `System.Windows.Documents.Table`: one `TableColumn` per (max) column count, single `TableRowGroup`, header row cells bold (cell + paragraph `FontWeight=Bold`), per-column `TextAlignment` carried from Markdig `ColumnDefinitions` where present. Ragged / header-only tables are total (no throw).
- **Image representation chosen: `Image.Tag` = source URI string + `AutomationProperties.SetName(image, alt)`; `Image.Source` left null (no `BitmapImage`, no fetch, no socket).** Image hosted in an `InlineUIContainer`. Relative/no-alt sources are total.
- Thematic break → `Paragraph` with `Tag="hr"` + thin bottom border (D1). Raw HTML (`HtmlBlock`/`HtmlInline`) → literal-text `Run`s, never parsed/executed/webview (D2). Hard line break → `LineBreak`; soft break → space (D4). Links/autolinks → inert `Hyperlink` with `NavigateUri` set, no click handler (D5, navigation is Story 3.5).
- Purity preserved: only Markdig + `System.Windows.*` used; no `System.Net.Http`, no AI, no `App`/`Agent` reference. `RenderingPurityTests` + inherited boundary/no-embedded-browser guards expected green.
- Verification is windows-latest CI only (no local .NET SDK / WPF on Linux). Markdig 1.3.1 AST API points (EmphasisInline.DelimiterChar/Count, TaskList.Checked, LeafBlock.Lines/StringLineGroup, Table/TableRow.IsHeader/ColumnDefinitions, LinkInline.IsImage/Url, HtmlInline.Tag) cross-checked against Markdig source before writing.

### File List

- `clients/windows/Rendering/FlowDocumentRenderer.cs` (NEW) — render API + shared Markdig pipeline + recursive block/inline mappers.
- `clients/windows/Rendering/FlowDocumentRenderOptions.cs` (NEW) — `MonospaceFontFamily`/`BodyFontFamily` seams (defaults Consolas / Segoe UI).
- (Test project + `Xunit.StaFact` + `AssemblyInfo.cs` `DisableTestParallelization` were already present from Step 4; not modified.)
- Untouched: `Rendering/MarkdownRenderer.cs`, `Rendering/TheMarkdownWeb.Rendering.csproj` (TFM/UseWPF/Markdig), all `App`/`Agent`/`App.Tests`, `TheMarkdownWeb.sln`, `build-windows.yml`.
