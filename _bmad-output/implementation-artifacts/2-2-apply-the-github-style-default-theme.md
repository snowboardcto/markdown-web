...
- [ ] **Task 1 — Author the GitHub-style stylesheet as a DESIGN-token CSS file** (AC: 1, 3, 4, 5)
...
- [ ] **Task 5 — Add Playwright theme + contrast tests (extend the existing harness)** (AC: 1, 2, 3, 4, 5)
  ...
  - [ ] Run the FULL suite — `cd web && npx playwright test` — and confirm **all prior 20 specs still pass** (no regression) PLUS the new theme/contrast/edge-case assertions. The existing structural specs (`ac1`, `ac2`, `ac3`, `ac5`, `ac6`) must remain green; if any breaks, the theme changed the semantic shell and must be corrected (theme is additive only). [Source: 2-1 story — 20/20 passing]
- [ ] **Task 6 — Final verification against ACs (Definition of Done)** (AC: 1, 2, 3, 4, 5)
  ...
  - [ ] Every text/surface pairing meets WCAG AA, with computed ratios recorded and the emitted-vs-target Shiki token colors confirmed (AC3) — the tight `#8250df`/`#cf222e` code tokens measured at their *rendered* value, not assumed.
  ...
  - [ ] Theme survives GFM edge cases: long code lines scroll inside the `<pre>` (no body horizontal overflow / measure blow-out), inline vs block code are distinct, nested blockquotes keep per-level rules, wide tables scroll/wrap within the column (AC5).
