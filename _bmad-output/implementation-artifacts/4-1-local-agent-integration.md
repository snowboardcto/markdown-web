...
- [x] **Task 1 — Grow the `Agent` module: `ILlmClient` + `LlmResult`, `ISecretStore`, `Persona`, `ReaderContext`, `PersonalizationResult`/`Outcome` (AC: 1, 3, 4)**
  - [x] Add `clients/windows/Agent/LlmResult.cs` — `public readonly record struct LlmResult { bool IsSuccess; string? Markdown; string? FailureReason; static Success(string); static Failure(string); }` (mirror `App.FetchResult`). [Source: AC1; App/MarkdownFetcher.cs FetchResult]
  - [x] Add `clients/windows/Agent/ILlmClient.cs` — `Task<LlmResult> CompleteAsync(string systemPrompt, string pageMarkdown, ReaderContext readerContext, CancellationToken ct)`; document the TOTAL contract (never throws). [Source: AC1, AC4]
  - [x] Add `clients/windows/Agent/ISecretStore.cs` — `string? GetApiKey(); void SetApiKey(string key); void ClearApiKey(); bool HasApiKey { get; }`; document "never logs the key". [Source: AC1, AC3]
  - [x] Add `clients/windows/Agent/Persona.cs` — the persona type + a static `Persona.Basic` (`Id="basic"`, `DisplayName="Basic"`, `IsPassThrough=true`, empty `SystemPrompt`). The ONLY seed persona at 4.1. [Source: AC1; architecture-epic4-agent.md D2 seed set — only Basic in 4.1]
  - [x] Add `clients/windows/Agent/ReaderContext.cs` — minimal-but-real reader context (`string? PreferredLanguage`, `string? PageUrl`); extensible. [Source: AC1, AC2]
  - [x] Add `clients/windows/Agent/PersonalizationResult.cs` — `public readonly record struct PersonalizationResult { string Markdown; PersonalizationOutcome Outcome; string? Notice; }` + `public enum PersonalizationOutcome { Transformed, PassThrough, NeedsKey, FellBack }`. `Markdown` is ALWAYS a renderable string. [Source: AC1, AC4]
  - [x] **`[Fact]` AC1:** the `Persona.Basic` constants; `LlmResult`/`PersonalizationResult` factory + default contracts (no WPF type → `[Fact]`, in `Agent.Tests`). [Source: AC1]

- [x] **Task 2 — `PersonalityEngine` (total) (AC: 1, 3, 4)**
  ...
  - [x] **`[Fact]` AC1:** pass-through (`Persona.Basic`) returns original markdown + `PassThrough`, fake `ILlmClient` asserted never called. [Source: AC1]
  - [x] **`[Fact]` AC1:** non-pass-through persona + fake returning canned transformed markdown → `Markdown == <canned>` + `Transformed`. [Source: AC1]
  - [x] **`[Fact]` AC3:** non-pass-through persona + in-memory fake store with NO key → `NeedsKey` + original markdown + notice; fake `ILlmClient` asserted never called. [Source: AC3]
  - [x] **`[Theory]`/`[Fact]` AC4:** failing fake / throwing fake / null-or-empty-returning fake / pre-cancelled token / oversized input → each returns total: no exception escapes, `Markdown == <original>`, `Outcome ∈ {NeedsKey, FellBack}`. [Source: AC4]
  - [x] **`[Fact]` AC3 (no leak):** with an in-memory store holding sentinel key `"sk-ant-SENTINEL"`, assert that string appears in NO surfaced `Notice`/`Markdown`/`FailureReason` across success + failure paths. [Source: AC3]

- [x] **Task 3 — `AnthropicLlmClient` (real provider call, total, injectable HttpClient) (AC: 1, 4, 5)**
  ...
  - [x] **`[Fact]` AC5 (success parse):** stub returns a canned Anthropic 200 (`{"content":[{"type":"text","text":"# Transformed"}]}`) → `LlmResult.Success` with `Markdown == "# Transformed"`. [Source: AC1, AC5]
  - [x] **`[Theory]`/`[Fact]` AC4 (totality):** stub 500 / wrong content-type / `throw HttpRequestException` / pre-cancelled token / no-key-in-store → each `LlmResult.Failure` (no throw). [Source: AC4]
  - [x] **`[Fact]` AC3 (no leak):** assert the sentinel key appears in NO `FailureReason` produced across the failure paths. [Source: AC3, AC5]

- [x] **Task 4 — `DpapiSecretStore` (real Windows DPAPI key store) (AC: 1, 3)**
...
- [x] **Task 5 — The App render-time seam: `PersonalizationGateway` + wire it into the render path (AC: 2)**
  ...
  - [x] Wire it in `clients/windows/App/MainWindow.xaml.cs`: compose `new AnthropicLlmClient(SharedHttpClient-or-its-own-client, new DpapiSecretStore())`, `new PersonalityEngine(llmClient, secretStore)`, `new PersonalizationGateway(engine, () => Persona.Basic)`. Update the render sink so the fetched markdown is resolved through the gateway before `ShowMarkdown` — e.g. the `NavigationController` render sink (line 49) becomes async-aware via the gateway, OR `FetchEndpointAsync`/the sink is adapted so `result.Markdown` flows through `ResolveMarkdownAsync` → `_contentHost.ShowMarkdown(resolved, pageUrl)`. **Keep `NavigationController`'s last-wins re-entrancy intact** (the gateway call is part of the same generation; a stale result is still dropped). `ContentHostController`/`FlowDocumentRenderer` UNCHANGED. [Source: AC2; MainWindow.xaml.cs lines 40–53; NavigationController.cs line 185]
  - [x] **`[Fact]` AC2 (gateway pass-through):** over `PersonalizationGateway` with `Persona.Basic` + a fake `ILlmClient` → `ResolveMarkdownAsync("# H\n\npara", uri, ct) == "# H\n\npara"`, fake LLM never called. [Source: AC2]
  - [x] **`[Fact]` AC2 (gateway transform):** with a non-pass-through persona + a fake returning canned markdown → returns the canned markdown. [Source: AC2]
  - [x] **`[StaFact]` AC2 (App renders the gateway output):** in `App.Tests`, construct a `ContentHostController` over a real `new FlowDocumentRenderer()` into a `FlowDocumentScrollViewer`; drive a fetched markdown through a `PersonalizationGateway` (Basic) → `ShowMarkdown(resolved, uri)`; assert `ContentScroll.Document` is a non-null `FlowDocument` with ≥1 block whose text is non-empty and reflects the original markdown (Basic pass-through). Construct-not-Show; no pixels. [Source: AC2; ContentHostTests patterns]

- [x] **Task 6 — `Agent.Tests` project + `.sln`/`ProjectReference` wiring (AC: 7)**
  - [x] Create `clients/windows/Agent.Tests/TheMarkdownWeb.Agent.Tests.csproj` mirroring `App.Tests`/`Rendering.Tests` (`net10.0-windows`, `UseWPF` not required unless a WPF type is touched — Agent.Tests touch none, so omit `UseWPF` or keep parity; `IsTestProject`; the same test-SDK/xunit/StaFact versions; a `ProjectReference` to `..\Agent\TheMarkdownWeb.Agent.csproj`). [Source: AC7]
  - [x] Add `clients/windows/Agent.Tests/AssemblyInfo.cs` with `[assembly: CollectionBehavior(DisableTestParallelization = true)]` (mirror App.Tests/AssemblyInfo.cs). [Source: AC7]
  ...
  - [x] Add a `ProjectReference` from `clients/windows/App/TheMarkdownWeb.App.csproj` to `..\Agent\TheMarkdownWeb.Agent.csproj` (App composes the engine). [Source: AC2, AC6]
  - [x] Add `Agent.Tests` to `clients/windows/TheMarkdownWeb.sln` (a new `Project(...)` entry + the four `GlobalSection(ProjectConfigurationPlatforms)` lines, mirroring the existing test-project entries; a fresh GUID). [Source: AC7]
  - [x] Confirm `build-windows.yml` is UNCHANGED — the `paths: clients/windows/**` filter already covers `Agent.Tests`; the `.sln` add brings it into restore/build/test. [Source: AC7]

- [x] **Task 7 — Purity / boundary / no-webview re-confirm + extend (AC: 6)**
  - [x] Confirm `Rendering` gains NO new `PackageReference` (still `{Markdig, ColorCode.Core}`), no `System.Net.*`/AI/webview, no App/Agent ref — `RenderingPurityTests` (allowlist + forbidden-substring incl. `Anthropic`/`OpenAI`/`WebView`) stays green. [Source: AC6]
  - [x] Extend `clients/windows/App.Tests/DependencyBoundaryTests.cs` with **`Agent_DoesNotReference_App`** (the `Agent` csproj has NO `ProjectReference` to `App`) and **`App_References_Agent`** (the `App` csproj declares a `ProjectReference` to `Agent`), mirroring the existing csproj-`ProjectReference` regex checks. Keep the existing `Rendering_DoesNotReference_AppOrAgent` + `App_References_Rendering` green. [Source: AC6; DependencyBoundaryTests.cs]
  - [x] Confirm `NoEmbeddedBrowserTests` (csproj glob across `clients/windows/**`) stays green — no `webview`/`cef`/`chromium`/… substring added to ANY csproj (Agent net additions, Agent.Tests). An LLM HTTP client is not a browser. [Source: AC6]

- [x] **Task 8 — STA / no-parallel discipline + final verification against ACs (Definition of Done — checked via CI, not locally) (AC: 1–7)**
  - [x] Confirm: engine/persona/keys/LLM-client tests are pure `[Fact]` (no `DispatcherObject`); ONLY AC2's App-seam `MainWindow`/`FlowDocument` test is `[StaFact]`; `Xunit.StaFact` + `DisableTestParallelization` present in every test project (added new to `Agent.Tests`). No shown `Window`/`Dispatcher` pump/socket/real `Process.Start`/real secret in logs/pixels/timing. [Source: AC7]
  - [x] **AC1:** the Agent module surface (`ILlmClient`/`AnthropicLlmClient`, `ISecretStore`/`DpapiSecretStore`, `PersonalityEngine`, `Persona.Basic`, pass-through) — proven by the `Agent.Tests` `[Fact]`s.
  - [x] **AC2:** the App render-time seam (fetch → `PersonalityEngine.PersonalizeAsync` → `FlowDocumentRenderer`) — proven by the gateway `[Fact]`s + the App-seam `[StaFact]`.
  - [x] **AC3:** BYO-key DPAPI storage (round-trip, never logged) + missing-key `NeedsKey` outcome — proven by the `DpapiSecretStore` smoke `[Fact]` + the `NeedsKey`/no-leak `[Fact]`s.
  - [x] **AC4:** graceful totality / fallback-to-original on every failure (no key, fail, throw, cancel, null/empty, oversized) — proven by the totality theory + the `AnthropicLlmClient` stub-handler totality `[Fact]`s.
  - [x] **AC5:** no server-side rewrite — the transform is a client→provider POST to `{BaseUrl}/v1/messages` carrying the reader's key; never a Markdown-Web host — proven by the stub-handler request-shape `[Fact]`s.
  - [x] **AC6:** Rendering pure + App→Agent→(API) boundary (Agent owns net+AI, no up-ref) — inherited purity/boundary/no-webview guards green + the new `Agent_DoesNotReference_App`/`App_References_Agent` `[Fact]`s.
...
- Grew the `Agent` stub into the real BYO-key module (namespace `TheMarkdownWeb.Agent`): `LlmResult`, `ILlmClient`, `ISecretStore`, `Persona` (+ `Persona.Basic`), `ReaderContext`, `PersonalizationOutcome`/`PersonalizationResult`, `PersonalityEngine`, `AnthropicOptions`, `AnthropicLlmClient`, `DpapiSecretStore`. Implemented to the EXACT test signatures (arg order, ctor shapes, factory contracts).
- `PersonalityEngine.PersonalizeAsync(string pageMarkdown, Persona persona, ReaderContext readerContext, CancellationToken ct)` is total: pass-through → `PassThrough` (zero LLM calls); oversized (> `MaxInputChars` = 8 MiB) → `FellBack` (no call); no key → `NeedsKey` (zero calls); cancellation honoured via `ct.ThrowIfCancellationRequested()` before trusting output → folds to `FellBack` + ORIGINAL; Success-with-usable-text → `Transformed`; Failure/blank/throw → `FellBack` + ORIGINAL. Catch-all guarantees no exception escapes. Key never placed in `Markdown`/`Notice`.
- `AnthropicLlmClient.CompleteAsync` issues exactly ONE `POST {BaseUrl}/v1/messages` with `x-api-key`=key, `anthropic-version`, `content-type: application/json`, body `{ model, max_tokens, system, messages:[{role:"user", content:<pageMarkdown>}] }` (System.Text.Json `Utf8JsonWriter`). No key → Failure, NO request. Parses first `content[].type=="text"` `.text`; blank/missing/malformed/non-2xx/`HttpRequestException`/cancel/timeout → Failure (key never in `FailureReason`). Per-request timeout via linked CTS (`AnthropicOptions.Timeout`, default 100s).
- `DpapiSecretStore` uses `ProtectedData.Protect/Unprotect` (CurrentUser); default path `%LOCALAPPDATA%\TheMarkdownWeb\agent.key`; writes ONLY ciphertext; Get/Has total (null/false when absent/undecryptable); Clear deletes. Added `System.Security.Cryptography.ProtectedData` 9.0.0 to the Agent csproj (pure managed MS package — no forbidden NoEmbeddedBrowser substring).
- App seam: `PersonalizationGateway(PersonalityEngine engine, Func<Persona> selectedPersona)` with `ResolveMarkdownAsync(string fetchedMarkdown, Uri pageUrl, CancellationToken ct)` (builds `ReaderContext` from `pageUrl`, returns `result.Markdown`, surfaces Notice/Outcome via `LastNotice`/`LastOutcome`). Wired in `MainWindow` (compose once; `() => Persona.Basic`). Seam placement = option (b): resolved inside `FetchEndpointAsync` so the agent output rides on `result.Markdown` into the controller's existing guarded render sink — `NavigationController`'s last-wins generation re-check (line 153) covers it, so a superseded navigation's resolved markdown is dropped. `ContentHostController`/`FlowDocumentRenderer`/`NavigationController` internals UNCHANGED. Basic pass-through → byte-identical to Epic-3.
- csproj: App → Agent ProjectReference added (App_References_Agent). Agent references Rendering only (kept; optional), never App (Agent_DoesNotReference_App). `Agent.Tests` was already in the `.sln`.
- Removed the legacy `Agent/IPersonality.cs` marker stub (superseded by `Persona`; no code referenced it).

### File List

- clients/windows/Agent/LlmResult.cs — NEW
- clients/windows/Agent/ILlmClient.cs — NEW
- clients/windows/Agent/ISecretStore.cs — NEW
- clients/windows/Agent/Persona.cs — NEW
- clients/windows/Agent/ReaderContext.cs — NEW
- clients/windows/Agent/PersonalizationResult.cs — NEW (enum + result record)
- clients/windows/Agent/PersonalityEngine.cs — NEW
- clients/windows/Agent/AnthropicOptions.cs — NEW
- clients/windows/Agent/AnthropicLlmClient.cs — NEW
- clients/windows/Agent/DpapiSecretStore.cs — NEW
- clients/windows/Agent/IPersonality.cs — REMOVED (superseded by Persona)
- clients/windows/Agent/TheMarkdownWeb.Agent.csproj — UPDATED (added ProtectedData 9.0.0 PackageReference; kept Rendering ProjectReference)
- clients/windows/App/PersonalizationGateway.cs — NEW
- clients/windows/App/MainWindow.xaml.cs — UPDATED (compose engine+gateway; resolve through gateway inside FetchEndpointAsync, last-wins preserved)
- clients/windows/App/TheMarkdownWeb.App.csproj — UPDATED (added App → Agent ProjectReference)
