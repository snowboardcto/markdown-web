...
- [x] **Task 1 — Pure `.md`-only URL validator in `App` (AC: 2)**
  ...
  - [x] (Optional helper, only if the address-bar display needs it) `public static bool TryGetHostPath(string input, out string hostPath)` to format the `host + path` shown in the bar (AC1) — keep pure. [Source: AC1 host/path display]

- [x] **Task 2 — `MarkdownFetcher` in `App` (App owns networking; injectable HttpClient) (AC: 4, 5, 6)**
...
- [x] **Task 4 — Address-bar view-model: validate → decline-or-fetch → state (AC: 2, 3, 4, 6)**
...
- Implemented the full App-side API surface exactly as pinned in the story / test contracts (namespace `TheMarkdownWeb.App`): `AddressBarValidation` (pure, total `IsLoadableMarkdownUrl` + optional `TryGetHostPath`), `FetchResult` (readonly record struct, `Success`/`Failure` factories), `MarkdownFetcher` (injectable `HttpClient`, `Accept: text/markdown` GET, never throws), `IUrlLauncher` + `SystemBrowserLauncher` (`Process.Start` shell-execute, try/catch), `AddressBarState` enum, and `AddressBarViewModel` (validate → decline-or-fetch → state machine, `INotifyPropertyChanged`).
- `SubmitAsync` sets `State = Loading` synchronously before the first await (satisfies the re-entrancy test that reads `Loading` while the gated fetch is still pending). Re-entrancy uses an `Interlocked` generation counter claimed before any await; only the latest submit may write a terminal state, so the stale first completion is ignored (last submit wins → `Loaded` "# Second"). No double terminal write, no crash.
- Decline path: `DeclinedUrl` is set to the trimmed raw input ONLY when it parses as an absolute http(s) Uri; non-URL / `mailto:` / `ftp:` / `file:` / `javascript:` declines leave `DeclinedUrl == null` and `OpenDeclinedInBrowser()` no-ops. No fetch is issued for any declined input (verified zero `SendCount`).
- Fetcher failure taxonomy → Failure (never throws): non-2xx (404/500), non-`text/markdown` 200, empty body, oversized body (8 MiB cap; the 64 MiB test trips the `Content-Length` guard before reading), `HttpRequestException`, and cancelled token (`OperationCanceledException`/`TaskCanceledException`).
- XAML: filled the reserved toolbar `Grid` column 1 with `AddressBar` (Border) → `LockIndicator` (TextBlock glyph, `AutomationProperties.Name="Secure"`), `AddressInput` (TextBox, `TabIndex=3` > Reload's 2, `Focusable`, tab-stop, two-way bound to `AddressText`, `AutomationProperties.Name="Address"`, Enter→`SubmitAsync`), and `MdOnlyTag` (TextBlock, `Text=".md only"` exact, non-empty `AutomationProperties.Name`). No `Button` added to the nav `StackPanel` (regression walker still collects exactly Back/Forward/Reload). Back(0)/Forward(1)/Reload(2) untouched.
- `MainWindow` constructs the real `AddressBarViewModel(new MarkdownFetcher(sharedHttpClient), new SystemBrowserLauncher())` and assigns it to `AddressBar.DataContext`; the window's `DataContext` stays `ShellViewModel` (3.1 tests intact). Fetched markdown is held on the VM, NOT rendered (Markdig render is Story 3.3).
- No new `PackageReference` (System.Net.Http ships with the framework / WPF). `NoEmbeddedBrowserTests` and `DependencyBoundaryTests` stay green: only `App`/`App.Tests` touched; Rendering/Agent untouched; no forbidden substring introduced; "open in system browser" is `Process.Start` shell-execute, not an embedded webview.
- Verification is exclusively `build-windows.yml` on windows-latest (Linux dev box has no .NET SDK; WPF is Windows-only). Code is written to compile clean and pass all new + existing tests; not built/run locally per the environment constraint. NOT committed.

### File List

New (clients/windows/App/):
- `AddressBarValidation.cs`
- `MarkdownFetcher.cs` (contains `FetchResult` + `MarkdownFetcher`)
- `IUrlLauncher.cs` (contains `IUrlLauncher` + `SystemBrowserLauncher`)
- `AddressBarViewModel.cs` (contains `AddressBarState` + `AddressBarViewModel`)

Modified (clients/windows/App/):
- `MainWindow.xaml` — filled the reserved column-1 address-bar slot (`AddressBar`/`LockIndicator`/`AddressInput`/`MdOnlyTag`).
